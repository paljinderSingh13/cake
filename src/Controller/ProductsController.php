    ...
    public function initialize()
{
 ...
 public function beforeFilter(Event $event){
      parent::beforeFilter($event);
       $this->Auth->allow(['appBrandListing','appUpdateCartDetail','appShowProduct','appProductWithPromotion','appHomepage','appShowSalesProduct','appProductdetail','appShowPrinterWithProduct','appShowPrinterWithBrandid','appShowProductWithPrinter','appAddtocart','showCartrecords','appRemoveCartitem','appPromocode','appShowPrinterWithBrandid','productSearch']);
}
...
$bannerArrS=array();
 ...
 foreach($productImages as $pikey=>$pivalue){
   ...
   $productSalesArr = array();
    foreach($query as $pkeys=>$pvalues){     
            $products['id'] = $pvalues->id;
            $products['product_name'] = $pvalues->product_name;
            $products['price'] = $pvalues->product_price_before_gst;
            $productImage = $this->Productimages->find('all',['conditions'=>[
                ['Productimages.product_id = '=>$pvalues->id]
             ],
             'fields'=>['Productimages.id','Productimages.product_id','Productimages.image'],
             'limit'=>1
          ]);
         ...
         $products['image'] = "";   
 foreach($productImage as $pikeys=>$pivalues){
            if(!empty($pivalues->image)){
                //localhost/DBR/img/product/14662622120.png
            $products['image'] = $this->request->scheme().'://'.$this->request->host().DS.$this->request->base.DS.'img'.DS.'product' . DS . $pivalues->image;
            }else{
            $products['image'] = "";   
            }
            
             }
            $productSalesArr[] = $products;
        }
        ...
        $arrdetail['showSalesClearanceProduct'] = $productSalesArr;
...
//        $query = $this->Products
//        ->find()->select(['id','product_name','product_price_before_gst'])
...
//        {
//          // echo $res->id;
//            $res->image = 'http://sgappstore.com//DBR/img/product/'.$res->productimages[0]->image;
//            unset($res->productimages);
//        }

// $arrdetail['error'] = '0';
//        $arrdetail['products'] = $response; 

//       }else{

//          $arrdetail['error'] = '1';
//          $arrdetail['products'] = "No record Found"; 
...
//        $arrdetail['error'] = '1';
//        $arrdetail['products'] = "Parameter Missing"; 
//    }
...
//_________START product Filter__________________________//
...
if($this->request->is('post'))
 ...
 //    ]);
    ...
    // $condition = 
    //     ['Products.product_name like' => '%'.$this->request->data['keyword'].'%',
    //     'OR' =>['Products.product_part_num like' => '%'.$this->request->data['keyword'].'%']
    //     ];
       ...
       //                  [
       //       'Products.product_name like' => '%'.$this->request->data['keyword'].'%',
       //       'Products.product_part_num like' => '%'.$this->request->data['keyword'].'%'
                             
       //                  ]
       //      ];
       
      //  'OR' =>array('Products.product_part_num like' => '%'.$this->request->data['keyword'].'%')

 $where['emp']=0;

         // compatability -  product_compatability<br>
        // price 
    if(isset($this->request->data['brand']) && $this->request->data['brand']!=""){
...
unset($where['emp']);
      ...
      if(isset($this->request->data['color']) && $this->request->data['color']!=""){
        unset($where['emp']);

        $where['product_color'] = $this->request->data['color'];
      }
if(isset($this->request->data['compatability']) && $this->request->data['compatability']!=""){
    unset($where['emp']);

        $where['product_compatability'] = $this->request->data['compatability'];
      }
      if(isset($this->request->data['price']) && $this->request->data['price']!=""){
       unset($where['emp']);
 
        $where['product_price_before_gst'] = $this->request->data['price'];
      }
      if(isset($this->request->data['partno']) && $this->request->data['partno']!=""){
        unset($where['emp']);

        $where['product_part_num'] = $this->request->data['partno'];
      }
       if(isset($this->request->data['product_description']) && $this->request->data['product_description'] !=""){
        unset($where['emp']);

        $where['product_description like'] = '%'.$this->request->data['product_description'].'%';
      }

      if(isset($this->request->data['keyword']) && $this->request->data['keyword']!=""){
     unset($where['emp']);
      $where['Products.product_name like'] = '%'.$this->request->data['keyword'].'%';
      }

if(isset($where['emp']) && $where['emp'] == 0)
{
    $query = $this->Products
    ->find()
    ->select(['id','product_name','product_price_before_gst'])
    ->contain(['Productimages']);
...
if(count($response)>0){
       ...
       // print_r($res);
         // echo $res->id;
        if(@$res->productimages[0]->image!=null)
        {
            $res->image = $this->request->scheme().'://'.$this->request->host().DS.$this->request->base.DS.'img'.DS.'product' . DS .$res->productimages[0]->image;
        }

            // $this->request->scheme().'://'.$this->request->host().DS.$this->request->base.DS.'img'.DS.'product' . DS .$res->productimages[0]->image;
            unset($res->productimages);
       }
       // die;
...
//_________END product Filter __________________________//
...
function showCartdetail($userid=0){ 
     ...
     $this->loadModel('Users');
             if($userid !=''){   
                $cartArr = array();
     $cartdetail = $this->Productcarts->find()->where(['Productcarts.user_id'=>$userid,'Productcarts.order_id'=>0])->toArray();
          if(!empty($cartdetail)){          
...
public function appRemoveCartitem(){
 ...
 Public function appUpdateCartDetail(){ 
       ...
       $this->loadModel('Productcarts');
        ...
        $arrdetail['myCart'] = $this->showCartdetail($this->request->data['user_id']);
                        }else{
         $arrdetail['error'] = '1';
